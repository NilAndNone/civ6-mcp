...
from civ_mcp import game_launcher  # noqa: E402
from civ_mcp.connection import GameConnection  # noqa: E402
from civ_mcp.game_lifecycle import load_game_save, save_game  # noqa: E402
...
def stop_repo_mcp_servers() -> str:
    """Stop stale repo-local civ-mcp server processes before direct observation."""
    if sys.platform != "win32":
        return "Repo-local civ-mcp cleanup is only implemented on Windows."

    ps = r"""
$ErrorActionPreference = 'Stop'
$rootInput = $env:CODEX_PHASE1_ROOT
if ([string]::IsNullOrWhiteSpace($rootInput)) {
    throw 'CODEX_PHASE1_ROOT is empty.'
}
$root = [System.IO.Path]::GetFullPath($rootInput).ToLowerInvariant()
$selfPid = [int]$env:CODEX_PHASE1_SELF_PID
$targets = Get-CimInstance Win32_Process | Where-Object {
    $_.ProcessId -ne $selfPid -and
    $_.CommandLine -and
    ($_.Name -in @('uv.exe', 'python.exe', 'civ-mcp.exe')) -and
    $_.CommandLine.ToLowerInvariant().Contains($root) -and
    ($_.CommandLine -match 'civ-mcp(\.exe)?' -or $_.CommandLine -match 'src[\\/]civ_mcp[\\/]server\.py')
}
$rows = @()
...
"""
    env = os.environ.copy()
    env["CODEX_PHASE1_ROOT"] = str(ROOT)
    env["CODEX_PHASE1_SELF_PID"] = str(os.getpid())
    result = subprocess.run(
        [
            "powershell",
            "-NoProfile",
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"Failed to stop repo-local civ-mcp servers: {details}")
    return (result.stdout or "No output from repo-local civ-mcp cleanup.").strip()
            ...
            gaps.append(
                {
                    ...
                    "next_step": "Set CIV_MCP_AGENT_MODEL or add a runtime metadata bridge if exact model provenance is required.",
                }
            )
...
async def run_short(args: argparse.Namespace) -> int:
    ...
    header = {
        ...
        "civ6_mcp_repo_commit": run_git(["rev-parse", "HEAD"]),
        "civ6_mcp_repo_branch": run_git(["branch", "--show-current"]),
        "civ6_mcp_repo_status": run_git(["status", "--short"]),
        "civ6_version_info": {
            "value": None,
            "gap": "No stable Civ6 version query is exposed by current civ6-mcp tools.",
        ...
        "codex_model_info": {
            "value": os.environ.get("CIV_MCP_AGENT_MODEL") or None,
            "gap": "Codex runtime model is not exposed to this external runner unless CIV_MCP_AGENT_MODEL is set.",
        },
        "save_name": save_name,
    ...
    if not header["codex_model_info"]["value"]:
        recorder.add_gap(
            ...
            "Set CIV_MCP_AGENT_MODEL or add a Codex runtime metadata bridge if this must be exact.",
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not stop repo-local civ-mcp server processes before the direct observation run.",
    )
    parser.add_argument(
        "--reuse-running-game",
        action="store_true",
