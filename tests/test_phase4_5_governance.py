...
from codex_hl.governance import automation
from codex_hl.phase3 import assets
from codex_hl.phase4 import improvements
from codex_hl.phase5 import scenarios
...
def make_asset_root(root: Path) -> Path:
    body = "# Phase 1 Observation\n\nKeep evidence complete.\n"
    asset_body = root / "assets" / "playbook-phase1.md"
            ...
            "assets": [
                {
                    ...
                    "asset_type": "playbook",
                    "version": "1.0.0",
                    "content_path": "assets/playbook-phase1.md",
                    "content_sha256": digest,
                    "source_refs": ["tests"],
                    "applicability": ["test"],
                    "risk_level": "high",
                    "rollback_path": "assets/playbook-phase1.md",
...
def make_confirmed_failure(workspace: Path, episode_id: str = "ep1") -> Path:
    phase2 = workspace / "episodes" / episode_id / "phase2"
    failure = {
        "failure_id": "fail-001",
    ...
    write_jsonl(phase2 / "failures.jsonl", [failure])
    write_jsonl(phase2 / "regression_seeds.jsonl", [seed])
    return phase2


def test_phase4_generates_candidate_packages_without_mutating_assets(tmp_path):
    asset_root = make_asset_root(tmp_path / "asset_root")
    make_confirmed_failure(tmp_path)
    before = (asset_root / "assets" / "playbook-phase1.md").read_text(encoding="utf-8")
    ...
    candidates = improvements.read_jsonl(tmp_path / "episodes" / "ep1" / "phase4" / "candidate_improvements.jsonl")
    ...
    assert candidate["target_asset"]["asset_id"] == "playbook.phase1_observation"
    assert "proposed_content" in candidate["proposed_change"]
    assert (asset_root / "assets" / "playbook-phase1.md").read_text(encoding="utf-8") == before
    assert (tmp_path / "episodes" / "ep1" / "phase4" / "phase4_review.html").exists()


def test_phase5_ingests_failures_into_idempotent_scenario_pool(tmp_path):
    make_confirmed_failure(tmp_path)
    pool_root = tmp_path / "validation" / "regression_scenarios"
    ...
    assert (tmp_path / "episodes" / "ep1" / "phase5" / "scenario_ingest.json").exists()
...
def test_governance_evaluates_applies_and_rolls_back_guarded_merge(tmp_path):
    ...
    candidate = improvements.read_jsonl(tmp_path / "episodes" / "ep1" / "phase4" / "candidate_improvements.jsonl")[0]
    ...
    asset_path = asset_root / "assets" / "playbook-phase1.md"
    ...
    assert "Phase 4 Candidate Lesson" in asset_path.read_text(encoding="utf-8")
