...
def test_real_strategy_asset_library_checks():
    result = assets.check_asset_library()

    assert result["ok"] is True
    assert "strategy.prompt.default_boundary" in result["asset_ids"]
...
def test_validate_catalog_rejects_sha_mismatch(tmp_path):
    ...
    with pytest.raises(assets.StrategyAssetError, match="content_sha256 mismatch"):
...
def test_validate_catalog_rejects_missing_ledger_coverage(tmp_path):
    ...
    with pytest.raises(assets.StrategyAssetError, match="change_ledger missing"):
        assets.validate_catalog(root)
...
def test_extract_t50_metrics_reads_db_only_episode(tmp_path):
    write_state(tmp_path, "db_ep", cities=3, techs=8, civics=6)
    episode = tmp_path / "episodes" / "db_ep"
    ...
    result = assets.extract_t50_metrics("db_ep", workspace=tmp_path)

    assert result["metrics"]["num_cities"] == 3
    assert result["metrics"]["completed_tech_count"] == 8
    assert result["state_path"].endswith("raw\\civ6_states\\state-0051-T0051-t50_final.json") or result[
        "state_path"
    ].endswith("raw/civ6_states/state-0051-T0051-t50_final.json")
