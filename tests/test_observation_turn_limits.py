...
def test_observation_observe_accepts_short_runs_t20_and_t50(monkeypatch, turns):
    module = load_runner_module()
    monkeypatch.setattr(sys, "argv", ["codex-hl-civ6-observe", "--turns", str(turns)])
...
def test_observation_observe_rejects_unsupported_turn_counts(monkeypatch, turns):
    module = load_runner_module()
    monkeypatch.setattr(sys, "argv", ["codex-hl-civ6-observe", "--turns", str(turns)])
...
def test_observation_observe_accepts_explicit_strategy_profile(monkeypatch):
    module = load_runner_module()
    monkeypatch.setattr(
        sys,
        "argv",
        [
            "codex-hl-civ6-observe",
            "--turns",
            "20",
            "--strategy-profile",
            "explore_scout_first",
...
def test_observation_observe_accepts_science_culture_t50_profile(monkeypatch):
    module = load_runner_module()
    monkeypatch.setattr(
        sys,
        "argv",
...
def test_observation_observe_accepts_candidate_package(monkeypatch, tmp_path):
    module = load_runner_module()
...
def test_observation_observe_accepts_golden_age_push_profile(monkeypatch):
    module = load_runner_module()
    monkeypatch.setattr(
        sys,
        "argv",
