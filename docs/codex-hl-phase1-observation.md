...
Phase 1 负责记录真实 Civ6 对局发生了什么。它只做观测和报告，不做失败
归因，不生成策略改进，不进入自动学习。
...
- Phase 3 active assets，用于记录本次运行使用的规则版本。
...
```bash
uv run pytest tests/test_phase1_human_report_contract.py tests/test_phase1_turn_limits.py -q
```
...
- 把 Phase 1 报告写成策略好坏判断。Phase 1 只能记录事实。
- 短跑结束后自动进入 T50。短跑后必须停下来等人工审阅。
- T50 后继续进入 Phase 2 或 T51+。T50 后同样必须停在报告。
