...
Phase 4 从 Phase 2 的正式 failure 生成候选改进包。它可以提出资产修改候选，
但不能直接修改资产。
...
只有人工确认后的正式 failure 可以进入 Phase 4。候选 failure 不可以。
...
- 把候选包当成已接受资产修改。
- 直接改 Phase 3 asset 正文。
- 没有保留当前资产 hash，导致后续 governance 无法判断候选是否过期。
- 在 Phase 4 里启动 Civ6 或 replay。
