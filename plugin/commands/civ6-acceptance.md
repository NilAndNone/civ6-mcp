...
现有证据，不启动 Civ6，不 replay，不修改 strategy asset。

## 输入

- `--candidate-package <candidate.json>`：Strategy Candidate 生成的候选 playbook 包。
...
```powershell
$env:PYTHONIOENCODING='utf-8'; & 'O:\civ6\.tools\uv\uv.exe' run codex-hl-civ6-acceptance `
  --candidate-package episodes\<source_ep>\strategy\candidates\candidate_packages\<candidate_id>\candidate.json `
...
- 候选包必须能回指到 Review 正式 `failures.jsonl`。
