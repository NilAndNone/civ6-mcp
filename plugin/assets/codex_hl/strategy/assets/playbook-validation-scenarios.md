# Validation Regression Scenarios Playbook

Validation turns real failures into a growing regression scenario pool.

Rules:

- Ingest only confirmed Review failures.
- Preserve source episode, source failure, evidence refs, save refs, expected behavior, and failed behavior.
- Write the shared pool under `validation/scenarios/` unless the caller explicitly chooses another pool root.
