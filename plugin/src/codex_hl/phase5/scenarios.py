...
PHASE_NAME = "Phase 5 Regression Scenario Pool"


class Phase5Error(RuntimeError):
    """User-facing Phase 5 error."""
...
def read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    for line_no, line in enumerate(path.read_text(encoding="utf-8-sig").splitlines(), start=1):
        ...
        try:
            ...
            raise Phase5Error(f"Invalid JSONL: {path}:{line_no}: {exc}") from exc
        if not isinstance(row, dict):
            raise Phase5Error(f"Expected object in {path}:{line_no}")
        rows.append(row)
    return rows
...
def build_scenario(episode_id: str, failure: dict[str, Any], seeds: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not save_refs:
        save_refs = [
            {"kind": "save", "note": "No direct save evidence; inspect Phase 1 save_index before making this runnable."}
        ]
    return {
        "schema_version": 1,
...
<head>
  <meta charset="utf-8">
  <title>Phase 5 Regression Scenarios</title>
  <style>
...
<body>
  <h1>Phase 5 Regression Scenario Pool</h1>
...
def ingest_episode_failures(
    ...
    *,
    ...
    pool_root = (pool_root or workspace / "validation" / "regression_scenarios").resolve()
    phase2_dir = workspace / "episodes" / episode_id / "phase2"
    failures = read_jsonl(phase2_dir / "failures.jsonl")
    if not failures:
        raise Phase5Error(f"No confirmed Phase 2 failures found: {phase2_dir / 'failures.jsonl'}")
    ...
    episode_phase5 = workspace / "episodes" / episode_id / "phase5"
    write_json(
        episode_phase5 / "scenario_ingest.json",
        {
            "schema_version": 1,
            "phase": PHASE_NAME,
            "generated_at": now_iso(),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest confirmed failures into the Phase 5 scenario pool.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    except (Phase5Error, OSError) as exc:
