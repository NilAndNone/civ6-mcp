...
def next_strategy_profile_from_phase2(
    *,
    ...
    if current_profile != GOLDEN_AGE_PUSH_STRATEGY_PROFILE and (
        ...
        reasons.append("phase2_t50_golden_age_priority_candidate")
...
def extract_t50_best_run_metrics(workspace: Path, episode_id: str) -> dict[str, Any]:
    base = phase3_assets.extract_t50_metrics(episode_id, workspace=workspace)
    state_path, state = phase3_assets._load_episode_final_state(episode_id, workspace=workspace)
...
def load_episode_strategy_trace(workspace: Path, episode_id: str) -> dict[str, Any]:
    ...
    decisions = read_jsonl(workspace / "episodes" / episode_id / "derived" / "decision_atoms.jsonl")
        ...
        manifest.update(
            {
                ...
                "phase2": list(resume_manifest.get("phase2") or []),
                ...
                "phase4": list(resume_manifest.get("phase4") or []),
                "phase5": list(resume_manifest.get("phase5") or []),
