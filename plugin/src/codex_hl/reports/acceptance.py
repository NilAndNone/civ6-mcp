...
def metric_summary(workspace: Path, episodes: list[str]) -> dict[str, Any]:
    rows = [strategy_registry.extract_t50_metrics(episode, workspace=workspace) for episode in episodes]
...
def episode_header(workspace: Path, episode_id: str) -> dict[str, Any]:
    return EpisodeReader(workspace / "episodes" / episode_id).read_json("header.json")


def episode_decisions(workspace: Path, episode_id: str) -> list[dict[str, Any]]:
    return EpisodeReader(workspace / "episodes" / episode_id).read_jsonl("derived/decision_atoms.jsonl")
...
def candidate_package_source_verified(workspace: Path, package: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode_id in source_episode_ids:
        path = workspace / "episodes" / episode_id / "review" / "failures.jsonl"
        if not path.exists():
            continue
...
def candidate_runtime_evidence(
    *,
    ...
    package_sha = strategy_registry.sha256_file(package_path)
