"""Offline Phase 2 v1 failure labeling for existing Phase 1 episodes.

This module deliberately never starts Civilization VI and never writes outside
``episodes/<episode_id>/phase2``.  It turns already-recorded Phase 1 evidence
...
class Phase2Error(RuntimeError):
    """User-facing Phase 2 validation error."""
...
class Phase1Bundle:
    ...
    @property
    def phase1_input_hash(self) -> str:
...
def read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    for line_no, line in enumerate(path.read_text(encoding="utf-8-sig").splitlines(), start=1):
        ...
        try:
            ...
            raise Phase2Error(f"Invalid JSONL in {path}:{line_no}: {exc}") from exc
        if not isinstance(value, dict):
            raise Phase2Error(f"Expected object in {path}:{line_no}")
        rows.append(value)
    return rows
...
def assert_under(path: Path, base: Path) -> None:
    try:
        ...
        raise Phase2Error(f"Refusing to write outside {base}: {path}") from exc


def atomic_write_text(path: Path, text: str, *, phase2_dir: Path | None = None) -> None:
    if phase2_dir is not None:
        assert_under(path, phase2_dir)
...
def write_json(path: Path, data: Any, *, phase2_dir: Path) -> None:
    atomic_write_text(path, json_dumps(data) + "\n", phase2_dir=phase2_dir)


def write_jsonl(path: Path, rows: list[dict[str, Any]], *, phase2_dir: Path) -> None:
    text = "".join(json.dumps(row, ensure_ascii=False, sort_keys=False) + "\n" for row in rows)
    atomic_write_text(path, text, phase2_dir=phase2_dir)


def append_jsonl(path: Path, row: dict[str, Any], *, phase2_dir: Path) -> None:
    assert_under(path, phase2_dir)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def build_phase1_input_manifest(
    episode_root: Path, human_baseline_path: Path | None = None
) -> dict[str, Any]:
    required = [
        episode_root / "header.json",
        episode_root / "raw" / "tool_calls.jsonl",
        episode_root / "raw" / "mcp.jsonl",
        episode_root / "raw" / "saves" / "save_index.jsonl",
        episode_root / "derived" / "decision_atoms.jsonl",
        episode_root / "derived" / "report_pack.json",
    ]
    state_files = sorted((episode_root / "raw" / "civ6_states").glob("*.json"))
    outcome_files = sorted((episode_root / "outcome").glob("phase1_*"))
    missing = [path_rel(path, episode_root) for path in required if not path.exists()]
    if not state_files:
        missing.append("raw/civ6_states/*.json")
    if not outcome_files:
        missing.append("outcome/phase1_*")
    if missing:
        raise Phase2Error(
            "Missing required Phase 1 inputs: " + ", ".join(sorted(missing))
        )
    ...
    if human_baseline_path is not None:
        if not human_baseline_path.exists():
            raise Phase2Error(f"Human baseline file not found: {human_baseline_path}")
    ...
    files_hash = hash_paths(phase1_files, episode_root)
    ...
    groups = {
        ...
        "raw": hash_paths(raw_files, episode_root),
        ...
        "outcome_phase1": hash_paths(outcome_files, episode_root),
    }
...
def load_phase1_bundle(
    ...
    *,
    ...
    if not episode_root.exists():
        raise Phase2Error(f"Episode not found: {episode_root}")
    phase2_dir = episode_root / "phase2"
    input_manifest = build_phase1_input_manifest(episode_root, human_baseline_path)
    header = read_json(episode_root / "header.json")
    report_pack = read_json(episode_root / "derived" / "report_pack.json")
    decisions = read_jsonl(episode_root / "derived" / "decision_atoms.jsonl")
    tool_calls = read_jsonl(episode_root / "raw" / "tool_calls.jsonl")
    mcp_events = read_jsonl(episode_root / "raw" / "mcp.jsonl")
    saves = read_jsonl(episode_root / "raw" / "saves" / "save_index.jsonl")
    states: dict[str, dict[str, Any]] = {}
    for state_path in sorted((episode_root / "raw" / "civ6_states").glob("*.json")):
        state = read_json(state_path)
        snapshot_id = str(state.get("snapshot_id") or state_path.stem)
...
def refs_for_decisions(bundle: Phase1Bundle, decisions: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def refs_for_report(bundle: Phase1Bundle, *, reason: str) -> list[dict[str, Any]]:
    return [
        {"kind": "episode", "id": bundle.episode_id},
...
def make_candidate(
    ...
    *,
    ...
    if capability_category not in CAPABILITY_CATEGORIES:
        raise Phase2Error(f"Invalid capability_category: {capability_category}")
    if failure_type not in FAILURE_TYPES:
        raise Phase2Error(f"Invalid failure_type: {failure_type}")
    if confidence not in CONFIDENCE_VALUES:
        raise Phase2Error(f"Invalid confidence: {confidence}")
    ...
    candidate = {
        ...
        "input_hash": bundle.phase1_input_hash,
...
def decision_turn_range(decisions: list[dict[str, Any]], bundle: Phase1Bundle) -> list[int | None]:
    ...
    if is_local_fragment_mode(bundle.episode_mode) and bundle.human_baseline_missing:
        candidates.append(
            make_candidate(
                ...
                confidence="low",
                confidence_rationale="Phase 1 report_pack 没有人类基线引用。",
    ...
    if hold_decisions:
        ...
        candidates.append(
            make_candidate(
                ...
                description=(
                    "短跑中单位行动多次选择驻守/hold。该行为可能只是 Phase 1 为避免推测性操作的保守边界，"
    ...
    if builder_decisions:
        ...
        candidates.append(
            make_candidate(
                ...
                description=(
                    "城市早期生产在可选侦察兵时选择建造者。该选择可能合理，但 Phase 2 v1 需要把"
    ...
    if mining_decisions:
        ...
        candidates.append(
            make_candidate(
                ...
                description=(
                    "Phase 1 记录显示科技选择来自固定早期优先级，而不是从当前地图资源、人类基线"
                    "或可见目标推导。Phase 2 只把它作为候选，不直接判定失败。"
                ),
                capability_category="planning",
                failure_type="misjudgment",
                confidence="low",
    ...
    if failed_tool_calls:
        ...
        candidates.append(
            make_candidate(
                ...
                title="Phase 1 工具错误需要确认是否影响 episode 可信度",
                description="Phase 1 证据中存在失败的高层工具调用；需要审阅它是否只是可恢复事件，还是影响结论的执行/验证失败。",
    ...
    if isinstance(evidence_status, dict):
        ...
        if failed_sections:
            candidates.append(
                make_candidate(
                    bundle,
                    rule_id="phase1_evidence_status_gap",
                    title="Phase 1 四类证据存在未通过项",
                    description="report_pack.evidence_status 中至少一项不是 PASS，Phase 2 只能生成 evidence_gap 候选。",
                    capability_category="verification",
                    failure_type="evidence_gap",
                    confidence="medium",
                    confidence_rationale=f"未通过项：{', '.join(failed_sections)}。",
                    evidence_refs=refs_for_report(bundle, reason="evidence_status_not_pass"),
                    expected_behavior="Phase 1 episode 应完整覆盖工具日志、状态快照、决策记录和存档关联。",
                    failed_behavior="至少一个 Phase 1 证据类别未通过本地报告契约。",
                    turn_range=[bundle.start_turn, bundle.final_turn],
                    source_analyzer_run_id=analyzer_run_id,
                    prompt_hash=prompt_hash,
                    session_id=session_id,
...
def build_analyzer_prompt(bundle: Phase1Bundle) -> str:
    run = bundle.report_pack.get("run", {})
    counts = bundle.report_pack.get("counts", {})
    return (
        "Phase 2 v1 rules-only analyzer input\n"
...
def archive_analyzer_run(
    ...
    *,
    ...
    run_dir = bundle.phase2_dir / "analyzer_runs" / run_id
    ...
    write_json(run_dir / "provenance.json", provenance, phase2_dir=bundle.phase2_dir)
    write_json(run_dir / "input_manifest.json", bundle.input_manifest, phase2_dir=bundle.phase2_dir)
    atomic_write_text(run_dir / "prompt.txt", prompt_text, phase2_dir=bundle.phase2_dir)
    write_json(run_dir / "llm_request.json", llm_request, phase2_dir=bundle.phase2_dir)
    write_json(run_dir / "llm_response.raw.json", llm_response, phase2_dir=bundle.phase2_dir)
    write_jsonl(run_dir / "candidate_failures.jsonl", candidates, phase2_dir=bundle.phase2_dir)
    write_json(run_dir / "dedupe_manifest.json", dedupe, phase2_dir=bundle.phase2_dir)
...
def validate_candidate(candidate: dict[str, Any]) -> None:
    ...
    if candidate["episode_mode"] not in EPISODE_MODES:
        raise Phase2Error(f"Invalid episode_mode in candidate {candidate['candidate_id']}")
    if candidate["capability_category"] not in CAPABILITY_CATEGORIES:
        raise Phase2Error(f"Invalid capability_category in candidate {candidate['candidate_id']}")
    if candidate["failure_type"] not in FAILURE_TYPES:
        raise Phase2Error(f"Invalid failure_type in candidate {candidate['candidate_id']}")
    if candidate["confidence"] not in CONFIDENCE_VALUES:
        raise Phase2Error(f"Invalid confidence in candidate {candidate['candidate_id']}")
    if candidate["source_kind"] not in {"heuristic", "llm", "merged"}:
        raise Phase2Error(f"Invalid source_kind in candidate {candidate['candidate_id']}")
    if candidate["human_status"] != "pending":
        raise Phase2Error(f"Candidate {candidate['candidate_id']} must start with human_status=pending")
    if not isinstance(candidate["evidence_refs"], list) or not candidate["evidence_refs"]:
        raise Phase2Error(f"Candidate {candidate['candidate_id']} needs evidence_refs")
    if not isinstance(candidate["scope_caveats"], list):
        raise Phase2Error(f"Candidate {candidate['candidate_id']} scope_caveats must be a list")


def validate_evidence_refs(bundle: Phase1Bundle, refs: list[dict[str, Any]]) -> None:
    ...
    for ref in refs:
        ...
        if kind == "episode":
            if ref_id != bundle.episode_id:
                raise Phase2Error(f"Evidence ref episode mismatch: {ref}")
        elif kind == "decision_atom":
            if str(ref_id) not in decision_ids:
                raise Phase2Error(f"Unknown decision evidence ref: {ref}")
        elif kind == "tool_call":
            if str(ref_id) not in tool_ids:
                raise Phase2Error(f"Unknown tool_call evidence ref: {ref}")
        elif kind == "mcp_event":
            if str(ref_id) not in mcp_ids:
                raise Phase2Error(f"Unknown mcp_event evidence ref: {ref}")
        elif kind == "state_snapshot":
            if str(ref_id) not in state_ids:
                raise Phase2Error(f"Unknown state_snapshot evidence ref: {ref}")
        elif kind == "save":
            if str(ref_id) not in save_ids:
                raise Phase2Error(f"Unknown save evidence ref: {ref}")
        elif kind == "report_pack":
            if not (bundle.episode_root / "derived" / "report_pack.json").exists():
                raise Phase2Error(f"Missing report_pack for evidence ref: {ref}")
        elif kind == "absent_evidence":
            if not ref.get("reason"):
                raise Phase2Error(f"absent_evidence ref requires reason: {ref}")
        else:
            raise Phase2Error(f"Unsupported evidence ref kind: {ref}")


def load_phase2_candidates(bundle: Phase1Bundle) -> list[dict[str, Any]]:
    path = bundle.phase2_dir / "candidates.jsonl"
    if not path.exists():
        raise Phase2Error(f"Missing candidates.jsonl. Run candidate generation first: {path}")
...
def load_confirmation_rows(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not candidate_id:
            raise Phase2Error(f"Confirmation row missing candidate_id: {row}")
        if action not in CONFIRMATION_ACTIONS:
            raise Phase2Error(f"Invalid confirmation action for {candidate_id}: {action}")
        if action == "modify" and not isinstance(row.get("modified_fields"), dict):
            raise Phase2Error(f"modify confirmation needs modified_fields object: {candidate_id}")
        if contains_forbidden_key(row):
            forbidden = contains_forbidden_key(row)
            raise Phase2Error(f"Confirmation row contains forbidden field {forbidden}: {candidate_id}")
    return rows
...
def build_failure(
    ...
    failure = {
        "failure_id": "fail_" + stable_id(candidate["candidate_id"], bundle.phase1_input_hash, length=12),
        ...
        "phase1_input_hash": bundle.phase1_input_hash,
        "source_candidate": {
    ...
    if forbidden:
        raise Phase2Error(f"Formal failure contains forbidden field: {forbidden}")
...
def find_start_save(bundle: Phase1Bundle, turn_range: list[Any]) -> tuple[str | None, str | None]:
...
def build_seed(bundle: Phase1Bundle, failure: dict[str, Any]) -> dict[str, Any]:
    ...
    seed = {
        "seed_id": "seed_" + stable_id(failure["failure_id"], bundle.phase1_input_hash, length=12),
        ...
        "not_runnable_in_phase2": True,
        "manual_review_required": True,
        "asset_change_allowed": False,
    }
    forbidden = contains_forbidden_key(seed)
    if forbidden:
        raise Phase2Error(f"Regression seed contains forbidden field: {forbidden}")
    if len(seed["source_failure_ids"]) != 1:
        raise Phase2Error("Phase 2 v1 requires exactly one source_failure_id per seed")
    return seed


def load_current_confirmations(bundle: Phase1Bundle) -> list[dict[str, Any]]:
    path = bundle.phase2_dir / "confirmation" / "confirmation.jsonl"
    return read_jsonl(path) if path.exists() else []


def load_formal_rows(bundle: Phase1Bundle) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    failures_path = bundle.phase2_dir / "failures.jsonl"
    seeds_path = bundle.phase2_dir / "regression_seeds.jsonl"
...
def analyzer_run_ids(bundle: Phase1Bundle) -> list[str]:
    runs_dir = bundle.phase2_dir / "analyzer_runs"
    if not runs_dir.exists():
        return []
    return sorted(path.name for path in runs_dir.iterdir() if path.is_dir())


def output_hashes(bundle: Phase1Bundle) -> list[dict[str, Any]]:
    if not bundle.phase2_dir.exists():
        return []
    rows = []
    for path in sorted(bundle.phase2_dir.rglob("*"), key=lambda p: path_rel(p, bundle.phase2_dir)):
        if not path.is_file() or path.name == "MANIFEST.json":
            continue
        rows.append(
            {
                "path": path_rel(path, bundle.phase2_dir),
...
def build_manifest(
    ...
    *,
    ...
    return {
        ...
        "phase2_write_boundary": "episodes/<episode_id>/phase2/",
        "forbidden_fields": sorted(FORBIDDEN_FIELDS),
    }
...
def write_manifest(
    ...
    *,
    ...
    write_json(bundle.phase2_dir / "MANIFEST.json", manifest, phase2_dir=bundle.phase2_dir)
...
def render_review_html(
    ...
    for run_id in analyzer_run_ids(bundle):
        provenance_path = bundle.phase2_dir / "analyzer_runs" / run_id / "provenance.json"
...
<head>
  <meta charset="utf-8">
  <title>Codex HL Civ6 Phase 2 审计快照 - {html.escape(bundle.episode_id)}</title>
  <style>
...
<body>
  <h1>Codex HL Civ6 Phase 2 审计快照</h1>
  <p class="muted">只读 Phase 1 episode；所有 Phase 2 产物位于 <code>episodes/{html.escape(bundle.episode_id)}/phase2/</code>。</p>
  <div class="summary">
  ...
  <p><strong>Phase 1 input hash</strong>: <code>{html.escape(bundle.phase1_input_hash)}</code></p>
...
def write_review(
    ...
    atomic_write_text(bundle.phase2_dir / "phase2_review.html", html_text, phase2_dir=bundle.phase2_dir)
...
def build_phase2_summary(
    ...
    md = [
        f"# Phase 2 v1 Summary: {bundle.episode_id}",
        "",
        f"- episode_mode: `{bundle.episode_mode}`",
        f"- turn_target: `{bundle.turn_target}`",
        f"- phase1_input_hash: `{bundle.phase1_input_hash}`",
...
def generate_candidates(
    ...
    *,
    ...
    bundle.phase2_dir.mkdir(parents=True, exist_ok=True)
    prompt_text = build_analyzer_prompt(bundle)
    prompt_hash = sha256_text(prompt_text)
    run_id = f"heuristic_{now_stamp()}_{stable_id(bundle.episode_id, bundle.phase1_input_hash, length=8)}"
    ...
    for candidate in candidates:
        validate_candidate(candidate)
        validate_evidence_refs(bundle, candidate["evidence_refs"])
    write_jsonl(bundle.phase2_dir / "candidates.jsonl", candidates, phase2_dir=bundle.phase2_dir)
    ...
    return {
        "episode_id": bundle.episode_id,
        "phase2_dir": str(bundle.phase2_dir),
        "candidates": len(candidates),
        "review_path": str(bundle.phase2_dir / "phase2_review.html"),
        "manifest_path": str(bundle.phase2_dir / "MANIFEST.json"),
    }


def ensure_phase1_hash_unchanged(bundle: Phase1Bundle) -> None:
    manifest_path = bundle.phase2_dir / "MANIFEST.json"
    if not manifest_path.exists():
        raise Phase2Error("Missing Phase 2 MANIFEST.json; generate candidates before apply.")
    previous = read_json(manifest_path)
    previous_hash = previous.get("phase1_input_hash") or (
        previous.get("input_manifest", {}).get("aggregate_sha256")
        if isinstance(previous.get("input_manifest"), dict)
        else None
    )
    if previous_hash and previous_hash != bundle.phase1_input_hash:
        raise Phase2Error(
            "Phase 1 input hash changed after candidate generation. "
            f"previous={previous_hash}, current={bundle.phase1_input_hash}"
        )
...
def apply_confirmation(
    ...
    *,
    ...
    ensure_phase1_hash_unchanged(bundle)
    candidates = load_phase2_candidates(bundle)
    ...
    for row in confirmation_rows:
        if row["candidate_id"] not in candidates_by_id:
            raise Phase2Error(f"Unknown confirmation candidate_id: {row['candidate_id']}")
    ...
    if len(seeds) != len(failures):
        raise Phase2Error("Phase 2 v1 requires one regression seed per failure")

    confirmation_dir = bundle.phase2_dir / "confirmation"
    ...
    write_jsonl(bundle.phase2_dir / "failures.jsonl", failures, phase2_dir=bundle.phase2_dir)
    write_jsonl(
        bundle.phase2_dir / "regression_seeds.jsonl",
        seeds,
        phase2_dir=bundle.phase2_dir,
    )
    summary_json, summary_md = build_phase2_summary(
        bundle, candidates, confirmation_rows, failures, seeds
    )
    write_json(bundle.phase2_dir / "phase2_summary.json", summary_json, phase2_dir=bundle.phase2_dir)
    atomic_write_text(
        bundle.phase2_dir / "phase2_summary.md",
        summary_md,
        phase2_dir=bundle.phase2_dir,
    )
    ...
    return {
        ...
        "review_path": str(bundle.phase2_dir / "phase2_review.html"),
        "summary_path": str(bundle.phase2_dir / "phase2_summary.md"),
    }
...
<head>
  <meta charset="utf-8">
  <title>Phase 2 Label - {html.escape(episode_id)}</title>
  <style>
    :root {{
      color-scheme: light;
      --ink: #18202a;
  ...
  <form class="shell" method="post" action="/confirm-batch">
    <section class="topbar">
      <div class="topline">
        ...
        <div class="actions">
          <a class="button" href="/phase2_review.html" target="_blank">审计快照</a>
...
def build_confirmation_row_from_form(
    ...
    if action not in CONFIRMATION_ACTIONS:
        raise Phase2Error(f"invalid action for {candidate_id}: {action}")
    modified_fields = json.loads(modified_raw or "{}")
    if not isinstance(modified_fields, dict):
        raise Phase2Error(f"modified_fields must be a JSON object: {candidate_id}")
    row = {
...
def run_label_server(bundle: Phase1Bundle, candidates: list[dict[str, Any]], *, idle_timeout: int) -> None:
    confirmation_dir = bundle.phase2_dir / "confirmation"
    confirmation_dir.mkdir(parents=True, exist_ok=True)
    candidate_ids = {candidate["candidate_id"] for candidate in candidates}
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if parsed.path == "/phase2_review.html":
                self._send(200, (bundle.phase2_dir / "phase2_review.html").read_text(encoding="utf-8"))
                return
            ...
            try:
                ...
                if parsed.path == "/confirm":
                    ...
                    if candidate_id not in candidate_ids:
                        raise Phase2Error(f"unknown candidate_id: {candidate_id}")
                    rows.append(
...
def load_confirmation_rows_from_values(rows: list[dict[str, Any]]) -> None:
    for row in rows:
        action = row.get("action")
        if action not in CONFIRMATION_ACTIONS:
            raise Phase2Error(f"Invalid confirmation action: {action}")
        if row.get("candidate_id") is None:
            raise Phase2Error("Confirmation row missing candidate_id")
        if action == "modify" and not isinstance(row.get("modified_fields"), dict):
            raise Phase2Error("modify confirmation needs modified_fields object")
        forbidden = contains_forbidden_key(row)
        if forbidden:
            raise Phase2Error(f"Confirmation row contains forbidden field: {forbidden}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--human-baseline",
        help="Optional human T10/T20 baseline file to include in the Phase 2 input manifest.",
    )
    parser.add_argument(
        "--session-id",
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        if args.label:
            ...
            candidates = read_jsonl(bundle.phase2_dir / "candidates.jsonl")
            run_label_server(bundle, candidates, idle_timeout=args.idle_timeout)
    except (Phase2Error, OSError, ValueError) as exc:
