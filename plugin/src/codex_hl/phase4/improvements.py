"""Phase 4 candidate improvement packages.

Phase 4 can propose asset changes from confirmed Phase 2 failures, but it must
...
PHASE_NAME = "Phase 4 Candidate Improvement Packages"
CAPABILITY_DIMENSIONS = phase3_assets.CAPABILITY_DIMENSIONS


class Phase4Error(RuntimeError):
    """User-facing Phase 4 error."""
...
def read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    for line_no, line in enumerate(path.read_text(encoding="utf-8-sig").splitlines(), start=1):
        ...
        try:
            ...
            raise Phase4Error(f"Invalid JSONL: {path}:{line_no}: {exc}") from exc
        if not isinstance(row, dict):
            raise Phase4Error(f"Expected JSON object in {path}:{line_no}")
        rows.append(row)
    return rows
...
def assert_under(path: Path, base: Path) -> None:
    try:
        ...
        raise Phase4Error(f"Refusing to write outside {base}: {path}") from exc
...
def select_target_asset(failure: dict[str, Any], catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    if not matching:
        matching = [asset for asset in active_assets if asset.get("asset_id") == "memory.low_trust_policy"]
    if not matching:
        raise Phase4Error(f"No active asset can receive capability {capability!r}")
...
def build_candidate_for_failure(
    ...
    *,
    ...
    target_path = phase3_assets.resolve_catalog_path(asset_root, target["content_path"])
        ...
        scenario_refs.append(
            {
                ...
                "note": "Phase 5 should convert this failure into a regression scenario before merge.",
            }
        )
    return {
        "schema_version": 1,
        ...
        "proposed_change": {
            ...
            "rationale": "Confirmed Phase 2 failure produced a candidate lesson for a matching active asset.",
        ...
        "asset_diff": {
            ...
            "current_sha256": target["content_sha256"],
            "proposed_sha256": proposed_sha,
            "diff_summary": f"Append one reviewed Phase 4 lesson to {target['asset_id']}.",
        },
        "validation_plan": {
            ...
            "side_effect_checks": [
                "No new forbidden Phase 2 fields.",
...
<head>
  <meta charset="utf-8">
  <title>Phase 4 Candidate Improvements - {html.escape(episode_id)}</title>
  <style>
...
<body>
  <h1>Phase 4 候选改进包</h1>
  <p>这些候选只来自已确认 Phase 2 failure，不会修改资产。进入 L4/L5 前必须完成多场景验证。</p>
...
def generate_candidate_packages(
    ...
    *,
    ...
    phase4_dir = workspace / "episodes" / episode_id / "phase4"
    failures = read_jsonl(phase2_dir / "failures.jsonl")
    if not failures:
        raise Phase4Error(f"No confirmed Phase 2 failures found: {phase2_dir / 'failures.jsonl'}")
    seeds = read_jsonl(phase2_dir / "regression_seeds.jsonl")
    ...
    assert_under(phase4_dir, workspace / "episodes" / episode_id)
    package_root = phase4_dir / "candidate_packages"
    for candidate in candidates:
    ...
    write_jsonl(phase4_dir / "candidate_improvements.jsonl", candidates)
    manifest = {
        "schema_version": 1,
        "phase": PHASE_NAME,
        ...
        "outputs": {
            ...
            "review": "phase4_review.html",
        },
    }
    write_json(phase4_dir / "MANIFEST.json", manifest)
    (phase4_dir / "phase4_review.html").write_text(render_review(candidates, episode_id), encoding="utf-8")
    return {
        "ok": True,
        "episode_id": episode_id,
        "phase4_dir": str(phase4_dir),
        "candidates": len(candidates),
        "review": str(phase4_dir / "phase4_review.html"),
    }


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Phase 4 candidate improvement packages.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    except (Phase4Error, phase3_assets.Phase3AssetError, OSError) as exc:
