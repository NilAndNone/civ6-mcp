"""Strategy asset registry validation and T50 metric comparison.
...
class StrategyAssetError(RuntimeError):
    """User-facing Strategy Asset validation error."""
...
def candidate_asset_roots() -> list[Path]:
    roots: list[Path] = []
    env_root = os.environ.get("CODEX_HL_CIV6_STRATEGY_ASSET_ROOT")
    if env_root:
        roots.append(Path(env_root).resolve())
    parents = list(MODULE_PATH.parents)
    if len(parents) > 3:
        roots.append(parents[3] / "assets" / "codex_hl" / "strategy")
    if len(parents) > 2:
        roots.append(parents[2] / "assets" / "codex_hl" / "strategy")
    roots.append(WORKSPACE_ROOT / "plugin" / "assets" / "codex_hl" / "strategy")
    roots.append(WORKSPACE_ROOT / "assets" / "codex_hl" / "strategy")
...
def read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise StrategyAssetError(f"Invalid JSON: {path}: {exc}") from exc
    if not isinstance(data, dict):
        raise StrategyAssetError(f"Expected JSON object: {path}")
    return data
...
def read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise StrategyAssetError(f"Missing JSONL file: {path}") from exc
    for line_no, line in enumerate(lines, start=1):
        ...
        try:
            ...
            raise StrategyAssetError(f"Invalid JSONL: {path}:{line_no}: {exc}") from exc
        if not isinstance(row, dict):
            raise StrategyAssetError(f"Expected JSON object in {path}:{line_no}")
        rows.append(row)
    return rows
...
def resolve_catalog_path(asset_root: Path, rel_path: str) -> Path:
    ...
    try:
        ...
        raise StrategyAssetError(f"Path escapes asset root: {rel_path}") from exc
    return candidate
...
def _require_string(row: dict[str, Any], key: str, *, context: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise StrategyAssetError(f"{context}.{key} must be a non-empty string")
    return value
...
def _require_string_list(row: dict[str, Any], key: str, *, context: str) -> list[str]:
    ...
    if not isinstance(value, list) or not value:
        raise StrategyAssetError(f"{context}.{key} must be a non-empty list")
    if not all(isinstance(item, str) and item.strip() for item in value):
        raise StrategyAssetError(f"{context}.{key} must contain only non-empty strings")
    return value
...
def load_catalog(asset_root: Path = ASSET_ROOT) -> dict[str, Any]:
    ...
    if not catalog_path.exists():
        raise StrategyAssetError(f"Missing Strategy Asset catalog: {catalog_path}")
...
def validate_catalog(asset_root: Path = ASSET_ROOT, *, require_ledger: bool = True) -> dict[str, Any]:
    ...
    if catalog.get("schema_version") != 1:
        raise StrategyAssetError("catalog.schema_version must be 1")
    if not isinstance(assets, list) or not assets:
        raise StrategyAssetError("catalog.assets must be a non-empty list")
    ...
    for index, asset in enumerate(assets):
        ...
        if not isinstance(asset, dict):
            raise StrategyAssetError(f"{context} must be an object")
        fields = set(asset)
        if fields != REQUIRED_ASSET_FIELDS:
            missing = sorted(REQUIRED_ASSET_FIELDS - fields)
            extra = sorted(fields - REQUIRED_ASSET_FIELDS)
            raise StrategyAssetError(f"{context} fields mismatch; missing={missing}; extra={extra}")

        asset_id = _require_string(asset, "asset_id", context=context)
        if asset_id in seen_ids:
            raise StrategyAssetError(f"Duplicate asset_id: {asset_id}")
        seen_ids.add(asset_id)

        asset_type = _require_string(asset, "asset_type", context=context)
        if asset_type not in ASSET_TYPES:
            raise StrategyAssetError(f"{asset_id}.asset_type must be one of {sorted(ASSET_TYPES)}")
        risk = _require_string(asset, "risk_level", context=context)
        if risk not in RISK_LEVELS:
            raise StrategyAssetError(f"{asset_id}.risk_level must be one of {sorted(RISK_LEVELS)}")
        status = _require_string(asset, "status", context=context)
        if status not in STATUSES:
            raise StrategyAssetError(f"{asset_id}.status must be one of {sorted(STATUSES)}")
        ...
        if invalid_dimensions:
            raise StrategyAssetError(f"{asset_id}.capability_dimensions invalid: {invalid_dimensions}")

        content_path = resolve_catalog_path(asset_root, _require_string(asset, "content_path", context=context))
        if not content_path.exists():
            raise StrategyAssetError(f"{asset_id}.content_path missing: {content_path}")
        actual_hash = sha256_file(content_path)
        if actual_hash != asset["content_sha256"]:
            raise StrategyAssetError(
        ...
        if not rollback_path.exists():
            raise StrategyAssetError(f"{asset_id}.rollback_path missing: {rollback_path}")
...
def validate_ledger(asset_root: Path, assets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row_index, row in enumerate(rows):
        ...
        if not isinstance(row.get("source_failure_ids"), list):
            raise StrategyAssetError(f"{context}.source_failure_ids must be a list")
        if not isinstance(row.get("source_episode_ids"), list):
            raise StrategyAssetError(f"{context}.source_episode_ids must be a list")
        if not isinstance(row.get("source_refs"), list) or not row["source_refs"]:
            raise StrategyAssetError(f"{context}.source_refs must be a non-empty list")
        if not isinstance(row.get("human_or_auto_judgments"), list) or not row["human_or_auto_judgments"]:
            raise StrategyAssetError(f"{context}.human_or_auto_judgments must be a non-empty list")
    ...
    if missing:
        raise StrategyAssetError(f"change_ledger missing asset/version coverage: {missing}")
    return rows
...
def active_asset_snapshot(asset_root: Path = ASSET_ROOT) -> dict[str, Any]:
    ...
    return {
        ...
        "workflow": "Strategy Asset System",
...
def _load_episode_final_state(episode_id: str, *, workspace: Path = WORKSPACE_ROOT) -> tuple[Path, dict[str, Any]]:
    ...
    for logical_path, state in reader.state_rows():
        try:
            path = episode_root / Path(logical_path)
        except TypeError:
    ...
    if not states:
        raise StrategyAssetError(f"No turn states found under: {episode_root / 'raw' / 'civ6_states'}")
...
def compare_t50_metrics(
    ...
    *,
    ...
    return {
        "schema_version": 1,
        "workflow": "Strategy T50 Metric Review",
        "generated_at": now_iso(),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate Codex HL Civ6 strategy assets.")
    parser.add_argument("--asset-root", type=Path, default=ASSET_ROOT)
    parser.add_argument("--check", action="store_true", help="validate the strategy asset catalog")
...
def main(argv: list[str] | None = None) -> None:
    ...
    except StrategyAssetError as exc:
