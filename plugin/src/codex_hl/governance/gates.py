...
WORKFLOW_NAME = "L4/L5 Guarded Asset Governance"
MAIN_METRICS = set(strategy_registry.NUMERIC_T50_METRICS)
...
def evaluate_gates(
    ...
    *,
    ...
    asset_path = strategy_registry.resolve_catalog_path(asset_root, asset["content_path"])
    current_hash = strategy_registry.sha256_file(asset_path)
...
def apply_merge(
    ...
    *,
    ...
    asset = strategy_registry.validate_catalog(asset_root, require_ledger=False)
    current_asset = find_asset(asset, target["asset_id"])
    asset_path = strategy_registry.resolve_catalog_path(asset_root, current_asset["content_path"])
    ...
    rollback_snapshot = {
        ...
        "previous_sha256": strategy_registry.sha256_file(asset_path),
    ...
    new_sha = strategy_registry.sha256_file(asset_path)
    ...
    updated_asset = update_catalog_asset(
        ...
        target["asset_id"],
        {
            ...
            "source_refs": sorted(
                ...
                | {f"strategy_candidate:{candidate['candidate_id']}", str(validation_report_path)}
            ),
    ...
    ledger_row = {
        ...
        "source_refs": [f"strategy_candidate:{candidate['candidate_id']}", str(validation_report_path)],
...
def rollback_from_audit(
    ...
    *,
    ...
    asset_path = strategy_registry.resolve_catalog_path(asset_root, str(snapshot["content_path"]))
    asset_path.write_bytes(str(snapshot["previous_content"]).encode("utf-8"))
    restored_sha = strategy_registry.sha256_file(asset_path)
...
def main(argv: list[str] | None = None) -> None:
    ...
    except (GovernanceError, strategy_registry.StrategyAssetError, OSError) as exc:
