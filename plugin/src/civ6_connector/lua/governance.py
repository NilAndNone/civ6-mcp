...
from civ6_connector.lua._helpers import (
    SENTINEL,
    _LUA_XP_THRESHOLD,
    _bail,
    _bail_lua,
...
from civ6_connector.lua.models import (
    AppointedGovernor,
    CityStateInfo,
    DedicationChoice,
    DedicationStatus,
...
def build_set_policies(assignments: dict[int, str]) -> str:
    ...
    for slot_idx, policy_type in assignments.items():
        ...
        cannot_slot_bail = _bail_lua(
            f'"ERR:CANNOT_SLOT|{policy_type} (" .. pType_{slot_idx} .. '
            f'") rejected for slot {slot_idx} (" .. sName_{slot_idx} .. ")"'
        )
        slot_mismatch_bail = _bail_lua(
