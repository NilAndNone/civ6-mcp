...
from civ6_connector.diary import (
...
from civ6_connector.logger import GameLogger
from civ6_connector.map_capture import MapCapture
from civ6_connector.spatial import SpatialTracker
from civ6_connector.spectator import CameraController, PopupWatcher
from civ6_connector.telemetry import (
    EVENT_CITY_ROW,
    EVENT_DIARY_ROW,
    AlertSink,
    CloudSink,
...
from civ6_connector.web_api import create_app
...
async def _auto_boot(conn: GameConnection, save_name: str) -> None:
    ...
    Called during lifespan when CODEX_HL_CIV6_SAVE_FILE is set (eval mode).
...
async def lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    cloud_bucket = os.environ.get("CODEX_HL_CIV6_TELEMETRY_BUCKET")
    if cloud_bucket:
        emitter.add_sink(CloudSink(cloud_bucket))
    alert_webhook = os.environ.get("CODEX_HL_CIV6_ALERT_WEBHOOK")
    if alert_webhook:
    ...
    eval_model = os.environ.get("CODEX_HL_CIV6_AGENT_MODEL", "")
    eval_metadata = os.environ.get("CODEX_HL_CIV6_METADATA", "")
    eval_scenario = ""
    if eval_metadata:
        try:
    ...
    save_file = os.environ.get("CODEX_HL_CIV6_SAVE_FILE")
    if save_file:
    ...
    *,
    ...
    try:
        ...
        if _logged._conn_errors >= 5:
            ...
            try:
                from civ6_connector.autosave import get_autosave_for_turn, get_latest_autosave
...
async def end_turn(
    ...
    # Model ID comes from CODEX_HL_CIV6_AGENT_MODEL env var (set by eval runner)
    env_model = os.environ.get("CODEX_HL_CIV6_AGENT_MODEL", "")
    if env_model:
...
def main():
    ...
    if os.environ.get("CODEX_HL_CIV6_DISABLE_LUA"):
