...
handle where the data actually lands:
  ...
  * LocalSink  — writes JSONL files to ~/.codex-hl-civ6/ (always on, backward compat)
...
from civ6_connector.version import GIT_DESCRIBE, GIT_SHA, VERSION

log = logging.getLogger(__name__)

LOCAL_DIR = Path.home() / ".codex-hl-civ6"
...
class LocalSink:
    """Writes JSONL files to ~/.codex-hl-civ6/, preserving existing file layout.
...
# ── Cloud sink (opt-in via CODEX_HL_CIV6_TELEMETRY_BUCKET) ─────────────────────
...
# ── Alert sink (opt-in via CODEX_HL_CIV6_ALERT_WEBHOOK) ────────────────────────
...
class TelemetryEmitter:
    ...
    def metadata(self) -> dict[str, Any]:
        """Eval metadata from CODEX_HL_CIV6_METADATA env var (or start() arg)."""
    ...
    def start(
        ...
        If run_id is None, reads CODEX_HL_CIV6_RUN_ID env var or generates a UUID.
        If metadata is None, reads CODEX_HL_CIV6_METADATA env var (JSON) or {}.
        """
        from civ6_connector.run_id import generate_run_id

        self._run_id = run_id or os.environ.get("CODEX_HL_CIV6_RUN_ID") or generate_run_id()
        if metadata is not None:
            self._metadata = metadata
        else:
            raw = os.environ.get("CODEX_HL_CIV6_METADATA", "")
            if raw:
                try:
                    self._metadata = json.loads(raw)
                except json.JSONDecodeError:
                    log.warning("Invalid CODEX_HL_CIV6_METADATA JSON: %s", raw)
                    self._metadata = {}
            else:
                self._metadata = {}
